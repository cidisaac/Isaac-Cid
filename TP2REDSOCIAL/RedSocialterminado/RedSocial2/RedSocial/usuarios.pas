...
const
        ...
        ErrSexoNoDefinido = 'Sexo no permitido. Puede ser masculino(M), femenino(F) o indefinido(I)';
        ErrNoExisteUsuario = 'No se encontr¢ ning£n usuario registrado con el nickname ingresado' ;
        ErrNoCoincidePass = 'Contrase§a incorrecta ';
        ErrNoPens = 'El usuario no ha publicado pensamientos';
...
Procedure InsertarUsuarioIdx(var UsuariosIdx:tUsuariosIdx; dato:tMail; ID,max:tIDusuario);

//formato [dd/mm/aaaa]
procedure validarFecha (cadena:string; var error:tError; var mensaje:tMensaje);

//lo pasa a minusculas
procedure validarSexoCaracter(var cadena:string; var error:tError; var mensaje:tMensaje);

//MaxLogUsuarios se inicializa en 0 y se devuelve con la cantidad de usuarios registrados
procedure GenerarIndicesUsuarios(var aUsuarios:taUsuarios; ruta:tRuta; var UsuariosIdxNickname,UsuariosIdxMail: tUsuariosIdx;var MaxIdx:tIDUsuario);
...
procedure validarNick(var cadena:string; var IdNick:tUsuariosIdx; maxIdx:tIDusuario; var error:tError; var mensaje:tMensaje);

procedure validarNombre(var cadena:string; var error:tError; var mensaje:tMensaje);

procedure validarCiudad(var cadena:string; var error:tError; var mensaje:tMensaje);

procedure validarMail(var cadena:string; var IdMail:tUsuariosIdx; maxIdx:tIDusuario;var error:tError; var mensaje:tMensaje);
...
procedure validarPass(var cadena:string; var error:tError; var mensaje:tMensaje);
...
procedure validarInteres(var pasados:tvIntereses;maxLog:tIntereses;var IDinteres:tIntereses;cadena:string;var listaIntereses : tListaIntereses; maxInteres:tIntereses; var error: tError; var mensaje:tMensaje);

//Debe venir validado el dato, la longitud y que sean caracteres validos. Y debe venir en minusculas
procedure validarIdxUnico(var aux:tMail; var usuariosIdx:tUsuariosIdx ; maxIdx:tIdUsuario; var error:tError; var mensaje:tMensaje);
...
begin
        ...
        while not (eof(aUsuarios)) do read(aUsuarios,rUsuario)      //ojo
end;
...
Procedure InicializarIdxUsuarios(var aUsuarios:taUsuarios;var rUsuario:trUsuario;var UsuariosIdxNickname,UsuariosIdxMail: tUsuariosIdx) ;
begin
...
var
        ...
        fin:boolean;
begin
        maxIdx:=MaxUsuario + 1;// si el archivo no existe o esta vacio
        fin := false;
        abrirArchivoUsuarios(aUsuarios, ruta);
        if (not EOF(aUsuarios)) then InicializarIdxUsuarios(aUsuarios,rUsuario,UsuariosIdxNickname,UsuariosIdxMail)
        else fin := true;
        while (not EOF(aUsuarios)) do
        begin
                read(aUsuarios,rUsuario);
                InsertarUsuarioIdx (UsuariosIdxNickname,rUsuario.Nickname,rUsuario.ID,rUsuario.ID-1);
                InsertarUsuarioIdx (UsuariosIdxMail,rUsuario.Mail,rUsuario.ID,rUsuario.ID-1)
        end;
...
Function AsignarIDusuario(maxIdx:tIDusuario):tIDusuario;
begin
        if (maxIdx <> MaxUsuario + 1 ) then AsignarIDusuario:=maxIdx+1
        else AsignarIDusuario:=0
end;
...
//debe venir la lista de intereses cargada y maxIntereses con el maximo logico de dicha lista
procedure validarInteres(var pasados:tvIntereses;maxLog:tIntereses;var IDinteres:tIntereses;cadena:string;var listaIntereses : tListaIntereses; maxInteres:tIntereses; var error: tError; var mensaje:tMensaje);
var
        i:byte;
begin
         //fijarse que sea de long valida y que este en la lista
         validarLongTexto(cadena,1,LongDescripcionInteres,error,mensaje);
         if (error=0) then
         begin
                error := 12;
                lowercase(cadena);
                i:= minIntereses;
                while (i <= maxInteres) and (error <> 0) do
                        if (cadena = listaIntereses[i]) then
                        begin
                                interesRepetido(pasados,i,maxLog,error,mensaje);
                                if (error=0) then IDinteres:= i              //devuelve la posicion del interes en intereses.dat
                        end
                        else inc(i)
         end
end;
...
begin
        ...
        begin
                ...
                if ((letra < 'a') or (letra > 'z')) and (letra <> ' ') and (letra <> '§') and (letra <> 'Å') and ( letra<> '†' ) and ( letra<> 'Ç' ) and ( letra<> '°' ) and ( letra<> '¢' ) and ( letra<> '£' ) and ( letra <> #39) then
                begin
                        error:=3;
                        mensaje :=ErrLetras
                end
...
begin
        ...
        begin
                medio := (min + max) div 2;
                if (aux = lowercase(usuariosIdx[medio].cadena)) then
                begin
                        mensaje := ErrIdxUnico;
                        error :=4
                end
                else
                        if (aux < lowercase(usuariosIdx[medio].cadena)) then max := medio - 1
                        else min := medio +1
        end
end;

//tomae en cuenta si el maximo es no valido porque no existe el archivo
procedure validarNick(var cadena:string; var IdNick:tUsuariosIdx; maxIdx:tIDusuario; var error:tError; var mensaje:tMensaje);
begin
        validarLongTexto(cadena,1,LongNombre,error,mensaje);
        if (error=0) and (maxIdx <> MaxUsuario + 1) then validarIdxUnico(cadena,IdNick,maxIdx,error,mensaje)
...
begin
        ...
        begin
                ...
                begin
                        ...
                        if (maxIdx <> MaxUsuario + 1) then  validarIdxUnico(cadena,IdMail,maxIdx,error,mensaje)
                end
        end
end;
...
begin
        error:=0;
        validarLongTexto(cadena,1,1,error,mensaje);
        if (error = 0 ) then
        begin
                cadena:= lowercase(cadena);
                if (cadena <> 'm') and (cadena <> 'f') and (cadena <> 'i') then
                begin
                        mensaje:= ErrSexoNoDefinido;
                        error:=11
                end
        end
end;
...
begin
        error:=0;
        validarLongTexto(coordenada,1,255,error,mensaje);
